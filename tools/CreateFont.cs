...
public class CreateFont : MonoBehaviour {
    ...
    // Resources目录下Fonts 目录每次只能放置一种字体制作
    // 导入图片设置 sprite 和 multiple
    ...
    static void CreateMyFontSprite()
    {
        ...
        {
            string selectionExt = Path.GetExtension(selectionPath); //扩展名 .png
            ...
            Sprite[] sprites = Resources.LoadAll<Sprite>("Fonts/");
            if (sprites.Length > 0)
            {

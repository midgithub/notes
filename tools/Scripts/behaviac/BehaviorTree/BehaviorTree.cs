...
[Hotfix]
    public abstract class BehaviorNode
    {
        ...
        void load_par(int version, string agentType, XmlNode node)
        {
            if (node.Name != "par")
            {
                Debug.Check(false);
            ...
            XmlAttribute eventParamAttr = node.Attributes["eventParam"];

            string name = node.Attributes["name"].Value;
            string type = node.Attributes["type"].Value.Replace("::", ".");
            string value = (valueAttr != null ? valueAttr.Value : null);
            string eventParam = (eventParamAttr != null ? eventParamAttr.Value : null);
            ...
            foreach (XmlNode c in node.ChildNodes)
            {
                if (!load_property_pars(ref properties, c, version, agentType))
                {
                    if (bNode)
                    {
                        ...
                        {
                            string pAttachClassName = c.Attributes["class"].Value;
                            ...
                            if (pAttachment != null)
                            {
                                ...
                                string idStr = c.Attributes["id"].Value;
        ...
        private bool load_property_pars(ref List<property_t> properties, XmlNode c, int version, string agentType)
        {
            if (c.Name == "property")
            {
                if (c.Attributes.Count == 1)
                {
                    XmlAttribute attr = c.Attributes[0];
            ...
            {
                foreach (XmlNode parNode in c.ChildNodes)
                {
                    if (parNode.Name == "par")
                    {
        ...
        protected static BehaviorNode load(string agentType, XmlNode node)
        {
    ...
    public class BehaviorTree : BehaviorNode
    {
		    ...
		    if (properties.Count > 0)
		    {
			    foreach (property_t p in properties)
			    {
				    ...
				    else if (p.name == "DescriptorRefs")
				    {
                        ...
                        if (this.m_descriptorRefs == null) return;
        ...
        */
        public bool load_xml(byte[] pBuffer)
        {
            ...
            //XmlNode rootNode = behaviorNode.ChildNodes[0];
            //BehaviorNode root = BehaviorNode.load(agentType, rootNode);

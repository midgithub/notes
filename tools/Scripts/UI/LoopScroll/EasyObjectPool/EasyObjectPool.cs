...
class Pool
{
    ...
    public Pool(string poolName, GameObject poolObjectPrefab, GameObject rootPoolObj, int initialCount, PoolInflationType type)
    {
        ...
        {
#if UNITY_EDITOR
            SG.LogMgr.LogError("[ObjPoolManager] null pool object prefab !");
#endif
            return;
        }
    ...
    public void ReturnObjectToPool(PoolObject po)
    {
        if (poolName.Equals(po.poolName))
        {
            ...
            {
#if UNITY_EDITOR
                SG.LogMgr.LogWarning(po.gameObject.name + " is already in pool. Why are you trying to return it again? Check usage.");
#endif
            }
            else
            {
        ...
        {
            SG.LogMgr.LogError(string.Format("Trying to add object to incorrect pool {0} {1}", po.poolName, poolName));
        }
    }
}

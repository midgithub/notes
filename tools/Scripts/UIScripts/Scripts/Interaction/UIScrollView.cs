...
public class UIScrollView : MonoBehaviour
{
    ...
    protected Vector3 mMomentum { get { return momentum; } set { if (momentum != value)CallBackDrag(); momentum = value; } }
    ...
    System.Action DragCallBack;
    public void SetDragCallBack(System.Action dragCallBack)
    { 
	...
	void LateUpdate ()
	{
		...
		if (mShouldMove && !mPressed)
		{
			...
			if (mMomentum.magnitude > 0.0001f)
			{
				...
				// Move the scroll view
                Vector3 tempmonent = mMomentum;
                Vector3 offset = NGUIMath.SpringDampen(ref tempmonent, 9f, delta);
                mMomentum = tempmonent;
				MoveAbsolute(offset);
        ...
        // Dampen the momentum
        Vector3 tempmonent1 = mMomentum;
        NGUIMath.SpringDampen(ref tempmonent1, 9f, delta);
        if (tempmonent1 != mMomentum)
            mMomentum = tempmonent1;
	}

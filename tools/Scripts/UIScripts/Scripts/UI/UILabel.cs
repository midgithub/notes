...
public class UILabel : UIWidget
{
		...
		if (mActiveTTF != fnt)
		{
			if (mActiveTTF != null)
			{
				...
				if (mFontUsage.TryGetValue(mActiveTTF, out usage))
				{
					...
					if (usage == 0)
					{
						//mActiveTTF.textureRebuildCallback = null;     //By XuXiang
			...
			if (mActiveTTF != null)
			{
				...
				if (!mFontUsage.TryGetValue(mActiveTTF, out usage))
                {
                    //mActiveTTF.textureRebuildCallback = OnFontTextureChanged; //By XuXiang
                }
	...
	public void SetCurrentSelection ()
	{
		...
		{
            string value = UIPopupList.current.value.Trim(); 
	...
	public void UpdateNGUIText ()
	{
        ...
        if(mFont != null)
        {
            if (!mFont.packedFontShader)
                isFont = true;
            defaul = mFont.defaultSize;
        }
        NGUIText.gradient = mApplyGradient && isFont;
		...
		NGUIText.fontScale = isDynamic ? mScale : ((float)mFontSize / defaul) * mScale;

...
public static class Localization
{
    ...
    static double mLsatReloadTime = 0;
    static public void ReloadData()
    {
#if UNITY_EDITOR
        if (Application.platform == RuntimePlatform.WindowsEditor)
        {
            //double nsinceTime = EditorApplication.timeSinceStartup;
            //int nframeCount = Time.frameCount;
            if (EditorApplication.timeSinceStartup - mLsatReloadTime > 1f)
            {
                reloadDictionary(PlayerPrefs.GetString("Language", "English"));  

                mLsatReloadTime = EditorApplication.timeSinceStartup;
            }
        }
#endif
    }

    static void reloadDictionary(string value)
    {
